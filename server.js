...
app.locals.title = 'Student Flashcards';
...
app.get('/api/v1/students/:id', (request, response) => {
    ...
    const foundStudent = app.locals.students.find(student => student.id === id)

    if(!foundStudent) {
        return response.status(404).json({ message: `Sorry, no student found with an id of ${id }` })
    }
...
app.post('/api/v1/students', (request, response) => {
    ...
    app.locals.students.push(submittedStudent)

    response.status(201).json(submittedStudent)
})

app.delete('/api/v1/students/:id', (request, response) => {
    const id = parseInt(request.params.id)
    const filteredStudents = app.locals.students.filter(student => student.id !== id)
    app.locals.students = filteredStudents
